...
The project **$apiName** is a Bonita REST API extension for **Bonita {short-bonita-version}** version #if("sp" == true)**entreprise** #else **community**#end written in `$language` language.
...
Take the built artifact `${artifactId}-${version}.zip` and upload it to your Bonita platform (see {doc-url}/rest-api-extension-archetype#_deployment[rest-api-extension deployment])
