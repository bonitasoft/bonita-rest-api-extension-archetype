...
println "[Integration Test] Test generation of kotlin sub module ${moduleArtifactId} under a parent that pins kotlin-maven-plugin, in folder ${parentFolder}"
...
// the managed kotlin-maven-plugin goes, but the kotlin dependencies still reference
// ${kotlin.version} so the property they share with the plugin must stay
def modulePom = modulePomFile.text
assert modulePom =~ /<kotlin\.version>[^<]+<\/kotlin\.version>/: 'kotlin.version property should be kept: the kotlin dependencies still reference it'
...
// Join the output pump threads so a failure is reported with a fully drained buffer
